...
class ConvNdRNNBase(torch.nn.Module):
    def __init__(self,
                 ...
                 in_channels: int,
                 out_channels: int,
                 kernel_size: Union[int, Sequence[int]],     # only conv rnn
                 num_layers: int=1,
                 bias: bool=True,
                 batch_first: bool=False,
                 dropout: float=0.,
                 bidirectional: bool=False,
                 convndim: int=2,                       # only conv rnn
                 stride: Union[int, Sequence[int]]=1,  # only conv rnn
                 dilation: Union[int, Sequence[int]]=1, # only conv rnn
                 groups: int=1):
        super().__init__()
        self.mode = mode
        ...
        self.kernel_size = ntuple(kernel_size)     #ntuple means that tuple with n times, see .utils
        ...
        self._all_weights = []       # the following loop to create the weights variables
...
class ConvRNNCellBase(torch.nn.Module):         
    ...
    def forward(self, input, hx=None):
        ...
        cell = _conv_cell_helper(
            ...
            stride=self.stride,
            dilation=self.dilation,
            groups=self.groups)   # _conv_cell_helper is the main difference between the rnn and convrnn
        if self.mode == 'PeepholeLSTM':
            return cell(
                input, hx,
